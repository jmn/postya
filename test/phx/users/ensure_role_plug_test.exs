defmodule PhxWeb.EnsureRolePlugTest do
  use PhxWeb.ConnCase

  alias PhxWeb.EnsureRolePlug
  ...
  setup do
    conn =
      ...
      |> Pow.Plug.put_config(otp_app: :phx)
      |> fetch_flash()
  ...
  test "call/2 with no user", %{conn: conn} do
    ...
    assert conn.halted
    assert Phoenix.ConnTest.redirected_to(conn) == Routes.post_path(conn, :index)
  end
  ...
  test "call/2 with non-admin user", %{conn: conn} do
    ...
    conn =
      conn
      |> Pow.Plug.assign_current_user(@user, otp_app: :phx)
  ...
  test "call/2 with admin user", %{conn: conn} do
    ...
    conn =
      conn
      |> Pow.Plug.assign_current_user(@admin, otp_app: :phx)
      |> EnsureRolePlug.call(opts)
